...
CString CSHA::GetHashString(BOOL bHex/* = FALSE*/, BOOL bURN/* = FALSE*/)
{
	SHA1 pHash;
	GetHash(&pHash);

	if (bHex)
		return HashToHexString(&pHash, bURN);
	else
		return HashToString(&pHash, bURN);
}

CString	CSHA::GetHashStringFromFile(LPCTSTR lpFilePath, BOOL bHex/* = FALSE*/, BOOL bURN/* = FALSE*/)
{
	Reset();
	...
	if (file.Open(lpFilePath, CFile::modeRead | CFile::shareDenyWrite)) {
		uReadCount = file.Read(buf, 8192);
		while (uReadCount) {
			Add(buf, uReadCount);
			uReadCount = file.Read(buf, 8192);
		}
	...
	return GetHashString(bHex, bURN);
}
