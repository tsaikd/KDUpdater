...
void CreateEmptyUTF16File(LPCTSTR lpPath)
{
	const static WORD BOM = 0xFEFF;
	HANDLE hFile = CreateFile(lpPath, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
/////////////////////////////////////////////////////////////////////////////////
CIni::CIni(LPCTSTR lpPathName/*= NULL*/)
	: m_pszPathName(NULL)
{
...
/*
	WritePrivateProfileStringW() does not write Unicode string if the ini
file is not exist or the ini file does not contain UTF-16LE BOM.
	Therefore, we should create a empty ini file that contains the BOM.
*/
#ifdef _UNICODE
		if (!PathFileExists(m_pszPathName))
			CreateEmptyUTF16File(m_pszPathName);
		else {
			HANDLE hFile = CreateFile(m_pszPathName, FILE_READ_DATA, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
			WORD testBOM = 0;
			DWORD dwRead;
			if (hFile != INVALID_HANDLE_VALUE) {
				ReadFile(hFile, &testBOM, 2, &dwRead, NULL);
				CloseHandle(hFile);
			}
			if (testBOM != 0xFEFF) {
				TCHAR bkFile[MAX_PATH];
				_stprintf(bkFile, _T("%s.UTF8.bak"), m_pszPathName);
				MoveFile(m_pszPathName, bkFile);
				CreateEmptyUTF16File(m_pszPathName);
			}
		}
#endif
...
{
	DWORD dwLen = m_pszPathName ? (DWORD) _tcslen(m_pszPathName) : 0;

	if (lpBuffer != NULL && dwBufSize >= dwLen) {
		if (dwLen > 0)
			_tcscpy(lpBuffer, m_pszPathName);
		else
			*lpBuffer = _T('\0');
	}
...
BOOL CIni::WriteArray(LPCTSTR lpSection, LPCTSTR lpKey, const CStringArray *pArray, int nWriteCount, LPCTSTR lpDelimiter) const
{
	...
	if (nWriteCount < 0)
		...
		nWriteCount = min(nWriteCount, (int) pArray->GetSize());
...
void CIni::GetUIntArray(LPCTSTR lpSection, LPCTSTR lpKey, CUIntArray* pArray, LPCTSTR lpDelimiter/* = NULL*/) const
{
	...
	CString tString,sub;
	tString = GetString(lpSection, lpKey);

	sub = tString.Tokenize(DELIMITER,curPos);
	while (!sub.IsEmpty()) {
		pArray->Add(_tcstoul(sub, NULL, __ValidateBase(BASE_DECIMAL)));
		sub = tString.Tokenize(DELIMITER, curPos);
	}
}

BOOL CIni::WriteUIntArray(LPCTSTR lpSection, LPCTSTR lpKey, const CUIntArray* pArray, int nWriteCount/* = -1*/, LPCTSTR lpDelimiter/* = NULL*/) const
{
	if (pArray == NULL)
		return FALSE;

	if (nWriteCount < 0)
		nWriteCount = (int) pArray->GetCount();
	else
	...
	for (int i = 0; i < nWriteCount; i++)
	{
		tInt.Format(_T("%u"), pArray->GetAt(i));
		sLine += tInt;
		if (i != nWriteCount - 1)
			sLine += DELIMITER;
	}
...
// Get a POINT value
POINT CIni::GetPoint(LPCTSTR lpSection, LPCTSTR lpKey, const POINT &ptDefault) const
{
	POINT pt;
...
// Get a RECT value
RECT CIni::GetRect(LPCTSTR lpSection, LPCTSTR lpKey, const RECT &rcDefault) const
{
	RECT rc;
...
BOOL CIni::WritePoint(LPCTSTR lpSection, LPCTSTR lpKey, const POINT &pt) const
{
	return WriteDataBlock(lpSection, lpKey, &pt, sizeof(POINT));
}

// Write a RECT to the ini file
BOOL CIni::WriteRect(LPCTSTR lpSection, LPCTSTR lpKey, const RECT &rc) const
{
...
BOOL CIni::DeleteEmptySection(LPCTSTR lpSection/* = NULL*/) const
{
	if (lpSection) {
		...
		BOOL bRes = TRUE;
		for (i=0 ; i<iCount ; i++)
			bRes = DeleteEmptySection(saSectionName[i]) && bRes;
		return bRes;
	}
}
#endif
...
{
	const DWORD LEN = (DWORD) ((INT_PTR)lpEnd - (INT_PTR)lpStart) / sizeof(TCHAR);
	LPTSTR psz = new TCHAR[LEN + 1];
...
//------------------------------------------------------------
// Reading and Writing Operations like DDX
//------------------------------------------------------------
void CIni::TransferInt(LPCTSTR lpSection, LPCTSTR lpKey, int &iValue, int iDefault, bool bSaveToFile)
{
	if (bSaveToFile)
		WriteInt(lpSection, lpKey, iValue);
	else
		iValue = GetInt(lpSection, lpKey, iDefault);
}

void CIni::TransferUInt(LPCTSTR lpSection, LPCTSTR lpKey, UINT &uValue, UINT uDefault, bool bSaveToFile)
{
	if (bSaveToFile)
		WriteUInt(lpSection, lpKey, uValue);
	else
		uValue = GetUInt(lpSection, lpKey, uDefault);
}

void CIni::TransferBool(LPCTSTR lpSection, LPCTSTR lpKey, bool &bValue, bool bDefault, bool bSaveToFile)
{
	if (bSaveToFile)
		WriteBool(lpSection, lpKey, bValue);
	else
		bValue = GetBool(lpSection, lpKey, bDefault) != 0;
}

void CIni::TransferRect(LPCTSTR lpSection, LPCTSTR lpKey, RECT &rcValue, const RECT &rcDefault, bool bSaveToFile)
{
	if (bSaveToFile)
		WriteRect(lpSection, lpKey, rcValue);
	else
		rcValue = GetRect(lpSection, lpKey, rcDefault);	
}

#ifdef __AFXWIN_H__
void CIni::TransferString(LPCTSTR lpSection, LPCTSTR lpKey, CString &sValue, LPCTSTR sDefault, bool bSaveToFile)
{
	if (bSaveToFile)
		WriteString(lpSection, lpKey, sValue);
	else
		sValue = GetString(lpSection, lpKey, sDefault);		
}
